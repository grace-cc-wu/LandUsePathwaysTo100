...
'''
#########################################################################
##---------------------ERASE + EXTRACT BY MASK------------------------###
#########################################################################
## nonenvironmental site suitability polygon output
solar = os.path.join(mainOutputFolder, "052518_siteSuitability.gdb\\solarPV_0_0_nonEnv_r1_cat1")
wind = os.path.join(mainOutputFolder, "052518_siteSuitability.gdb\\wind_0_03_nonEnv_r3_cat1")

## exclusions
cat1_ex_solar = os.path.join(mainInputFolder,"envData\Cat1_solar\Cat1_u_d_s.shp")

cat2_ex_solar = {1: os.path.join(mainInputFolder,"envData\\Cat2\\solar_p1\\Solar_p1.shp"), 
           2: os.path.join(mainInputFolder,"envData\\Cat2\\solar_p2\\Solar_p2.shp"),
           3: os.path.join(mainInputFolder,"envData\\Cat2\\both_p1\\Both_p1.shp"),
           4: os.path.join(mainInputFolder,"envData\\Cat2\\both_p2\\Both_p2.shp"),
           5: os.path.join(mainInputFolder,"envData\\Cat2\\0045_AHPRC_Cat2\\0045_AHPRC\\data\\v101\\nps_identified_high_potential_for_resource_conflict.gdb\\NPS_AHPRC")}

cat3_ex_solar = os.path.join(mainInputFolder,"envData\\Cat3\\Cat3_solar_excl_base.shp")

cat4_ex = os.path.join(mainInputFolder,"envData\\Cat4\\Cat4_u_d_s.shp")

## Create temp GDB to hold intermediate files:
tempGDBpath = os.path.join(mainOutputFolder,dateAnalysis + "_temp_resourceAssessment.gdb") ## Name of the fgdb to store outputs 
tempGDBname = dateAnalysis + "_temp_resourceAssessment.gdb" ## here re-write the name of the file geodatabase
if not(arcpy.Exists(tempGDBpath)): # Create new fgdb if one does not already exist
    print("creating fgdb " + tempGDBpath)
    arcpy.CreateFileGDB_management(mainOutputFolder, tempGDBname)

#### UPDATES TO ENV SCREENS
## exclusions to apply in Cat 1:
tnc_cat1 = os.path.join(mainInputFolder, "envData\\tnc_lands_cat1_2\\tnc_lands_cat1_easements_proj.shp")
## exclusions to apply in Cat 1:
tnc_cat1_2 = os.path.join(mainInputFolder, "envData\\tnc_lands_cat1_2\\tnc_lands_cat1_2_merged_proj.shp")

ex_ls = {"Cat1" : tnc_cat1, "Cat2" : tnc_cat1_2,\
         "Cat3" : tnc_cat1_2, "Cat4" : tnc_cat1_2}
    
    
## Env Site suitability inputs for PV
ft_ls = {"Cat1" : "solarPV_0_0_nonEnv_r1_cat1_int",\
        "Cat2" : "solarPV_0_0_nonEnv_r1_cat2e_int",\
        "Cat3" : "solarPV_0_0_nonEnv_r1_cat3_int", \
         "Cat4": "solarPV_0_0_nonEnv_r1_cat4_int"}
CFraster = solarCF

## WIND:
ft_ls = {"Cat1" : "wind_0_03_nonEnv_r3_cat1_int_singlepart",\
        "Cat2" : "wind_0_03_nonEnv_r3_cat2e_int_singlepart",\
        "Cat3" : "wind_0_03_nonEnv_r3_cat3c_int", \
         "Cat4": "wind_0_03_nonEnv_r3_cat4_int"}
CFraster = windCF

## assumptions:
minArea = 1 # km2
selectSuffix = "_gt1km2"
## Erase, convert multipart to singlepart, recalculate area, then select areas greater than 1 or 2 km2

for cat in ft_ls:
    ft =  os.path.join(mainOutputFolder, "061218_temp_resourceAssessment.gdb", ft_ls[cat])
    
    ## erase
    print("Erasing " + str(ft_ls[cat]))
    ft_erased_file = os.path.join(mainOutputFolder, tempGDBname, ft_ls[cat])
    ft_erased = arcpy.Erase_analysis(ft, os.path.join(mainInputFolder, ex_ls[cat]), ft_erased_file)

    ## convert multipart to singlepart
    print("Converting from multipart to singlepart")
    ft_singlept_file = os.path.join(mainOutputFolder, tempGDBname , ft_ls[cat] + "_singlepart")
    ft_singlept = arcpy.MultipartToSinglepart_management(in_features = ft_erased_file, out_feature_class = ft_singlept_file)
    ...
    print("Done: select min area " + ft_singlept_file + selectSuffix + "_gt1km2")   
'''
